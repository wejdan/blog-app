    ...
    <Transition in={isOpen} timeout={300} unmountOnExit>
      {(state) => (
        ...
        >
          <div
            ref={modalContentRef}
            className={`w-52 bg-gray-500 dark:bg-gray-700 shadow-lg relative rounded-md`}
...
function Open({ children }) {
  ...
  const handleClick = (e) => {
    ...
    const y = buttonRect.bottom + window.scrollY + buttonRect.height / 3;
...
function Item({ onClick, icon, children }) {
  ...
  return (
    <div
      ...
      className="cursor-pointer h-12 px-5 flex items-center dark:bg-gray-700 text-white dark:hover:bg-gray-800 hover:bg-slate-100 transition-all duration-200 w-full "
    >
